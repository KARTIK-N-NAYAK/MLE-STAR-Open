...
class _InMemorySessionService:
    def __init__(self):
        print("Initializing _InMemorySessionService...")
        self._sessions = {}
        self._counter = 0

    async def create_session(self, app_name: str, user_id: str) -> _Session:
        print(f"Creating session for app: {app_name}, user_id: {user_id}...")
        self._counter += 1
...
class InMemoryRunner:
    ...
    async def run_async(
        ...
        print(f"Running agent asynchronously for user_id: {user_id}, session_id: {session_id}...")
